...
public static class Compatibility
{
    ...
    public static bool IsMethodConsistent(MethodInfo methodInfo, Type[] correctMethodTypes, bool logError = false, string modPackageIdForLog = null)
    {
        ...
        if (array.Length != correctMethodTypes.Length)
        {
            if (logError)
                Log.Error((modPackageIdForLog == null ? "Failed to support a mod: " : "Failed to support " + GetModName(modPackageIdForLog) + ": ")
                        + "Inconsistent number of parameters for method '" + (methodInfo.ReflectedType is null ? "" : methodInfo.ReflectedType.FullName + ".")
                        + methodInfo.Name + "'");
            return false;
        }
        for (int i = 0; i < array.Length; i++)
            if (array[i] != correctMethodTypes[i])
            {
                if (logError)
                    Log.Error((modPackageIdForLog == null ? "Failed to support a mod: " : "Failed to support " + GetModName(modPackageIdForLog) + ": ")
                            + $"Inconsistent parameter {i + 1} for method '{(methodInfo.ReflectedType is null ? "" : methodInfo.ReflectedType.FullName + ".") + methodInfo.Name}'"
        ...
        string methodNameForLog = null)
    {
        ...
        if (!logError || modName == null)
            return null;
        Log.Error("Failed to support " + modName + ": Couldn't get " + (methodNameForLog == null ? "an unknown method" : "method " + methodNameForLog) + "!");
        return null;
    }
    ...
    public static MethodInfo GetConsistentMethod(string modPackageId, string typeColonMethodName, Type[] correctMethodTypes, bool logError = false)
        => GetConsistentMethod(modPackageId, GetMethod(typeColonMethodName, correctMethodTypes) ?? GetMethod(typeColonMethodName), correctMethodTypes, logError,
            typeColonMethodName);
}